...
void OBCameraNode::publishDepthPointCloud(const std::shared_ptr<ob::FrameSet> &frame_set) {
  ...
  if (valid_count == 0) {
    ...
    sensor_msgs::PointCloud2Iterator<float> x(*empty_msg, "x");
    sensor_msgs::PointCloud2Iterator<float> y(*empty_msg, "y");
    sensor_msgs::PointCloud2Iterator<float> z(*empty_msg, "z");
    ...
    *z = std::numeric_limits<float>::quiet_NaN();
    ...
    if (!cloud_frame_id_.empty()) frame_id = cloud_frame_id_;
